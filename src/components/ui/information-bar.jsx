...
const InformationBar = ({ bmi = 0, variant, className, ...props }) => {
  return (
    <div className={informationBarVariants({ variant, className })} {...props}>
      {variant === 'default' ? (
        <>
          <Typography type="preset-4">Welcome!</Typography>
          <Typography type="preset-7">
        ...
        <>
          <div className="flex flex-col gap-2 w-1/2">
            <Typography type="preset-6">Your BMI is...</Typography>
            <Typography type="preset-1">
              {bmi === 0 ? bmi : bmi.toFixed(2)}
            </Typography>
          </div>

          <Typography type="preset-7 w-1/2">
